<template>
	<view class="container">
	  ...
	  <view class="content">
		<view class="wrap">
			<!-- <view class="noticebar"  @tap.stop="handleInstructionManual">
				复制1~3样音&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;&nbsp;发客服试音&nbsp;&nbsp;&nbsp;>&nbsp;&nbsp;&nbsp;付款录成品
		    </view>	 -->
	  ...
	  <u-popup v-model="collectShow" mode="bottom" border-radius="24">
		  <view class="collect_item" @click="downloadcopySampleVoices(operationItem,'复制样音信息成功')">
			  <image
			      class="collection"
	  ...
	  <view v-if="speedPromptShow" class="masklayer">
		  <view class="coupons_box">
			<image
				class="shutdown_icon"
				src="@/static/coupons/shutdownC.png"
				mode="scaleToFill"
				@click="handleshutdownC"
			/>
			<image
				class="coupons_icon"
				src="@/static/img/orderprobule.png"
				mode="scaleToFill"
			/>
			<view class="coupons_content">
				<view class="content_text content_text_blue">
					<view class="text">1、普通类型需求，可选【自主按样下单】，<text class="red_text">享受9折优惠</text>，告别频繁沟通、所听即所得，成品若有问题售后客服一对一服务</view>
					<view class="text">2、先挑选样音再试音，<text class="red_text">满意率高达90%</text></view>
					<!-- <view class="text">3、<text class="bule_text">50元内预算</text>，暂不支持试音，请选择【自主按样下单】</view> -->
				</view>
				<view class="know_btn_red" @click="handleshutdownC">我知道了</view>
			</view>
		</view>
	  </view>		  
	</view>
...
import { servicetag, servicsecList, collection, loginPopup } from "@/api/voice.js"
...
import uniCopy from '@/utils/uni-copy.js'
	export default {
		...
		onLoad(options) {
			this.current = 0
			if(this.speedPrompt_status === 0) {
			   this.speedPromptShow = false
			}else {
			  this.speedPromptShow = true
			  loginPopup().then((res)=>{
                 console.log('每次见两次', res)
			   })
			}
			this.getUnionid()
		},
