	  ...
	  <u-popup v-model="collectShow" mode="bottom" border-radius="24">
		  ...
		  <view class="collect_item">
			  ...
			  <view class="cloudsharebox">
				<view class="collect_text Csharebtn" @click="operationItem.share_type===0 ? handlejoincloudAdd(operationItem) :  handlejoincloud(operationItem)">{{operationItem.share_type===0 ? '加入云分享' : '移除云分享'}}</view>
                <view class="cloud_btn">
...
import { collection, loginPopup } from "@/api/voice.js"
import { teacherdetail,shareServicedata,shareDel,shareid,shareAdd } from "@/api/index.js";
...
import uniCopy from '@/utils/uni-copy.js'
	export default {
		...
		async onShareAppMessage(res) {
			...
			let id = ''
			await shareid().then((res)=>{
				console.log('请求的id',res)
				id = res.data
			})
			return {
				title: '云分享',
				desc: '',
				path: "/subpkg/pages/cshare/cshare?id="+id,
		...
		methods: {
		  ...
		  async getVoicelist() {
				...
				const res = await shareServicedata(this.tagParameter)
