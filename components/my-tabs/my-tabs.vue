			...
			<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="u-scroll-box" :class="{'u-tabs-scorll-flex': !isScroll}">
					<view class="u-tab-item u-line-1" :id="'u-tab-item-' + index" v-for="(item, index) in list" :key="index" @tap="clickTab(index)"
					 ...
					 <view v-if="modifyShow" class="modifybtn" @tap.stop="handleRevision($event,item)">
						 <image
						     class="modifybtn_icon"
							 src="@/static/carddetails/modifybtnw.png"
							 mode="scaleToFill"
						 />
						 <text class="modifybtn_text">修改</text>
					 </view>
					<u-badge :count="item[count] || item['count'] || 0" :offset="offset" size="mini"></u-badge>
						{{ item.tags[0].value || item.tags[0].value }}
                    <view v-if="item.admin_price!= ''" :style="[tabSubItemStyle(index)]" class="u-table-subitem">{{item.admin_price}}</view>
					<view v-else :style="[tabSubItemStyle(index)]" class="u-table-subitem">见稿报价</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			handleRevision(e,item) {
			   e.preventDefault();
			   const items = JSON.stringify(item);
               uni.navigateTo({ url: '/subpkg/pages/editbasic/editbasic?type=2&item='+encodeURIComponent(items)+'&id='+ this.token})	
			},
			// 查询tab的布局信息
			getTabRect() {
				// 创建节点查询
...
<style lang="scss" scoped>
	...
	.u-tab-item {
		...
		.modifybtn {
			...
			width: 85%;
			height: 100%;
			background: RGBA(30, 120, 251, 0.7);
			.modifybtn_icon {
              width: 23.551rpx;
			  height: 23.551rpx;
			}
